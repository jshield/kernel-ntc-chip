...
RUN apt-get update && apt-get install -y crossbuild-essential-armhf bc libssl-dev bison flex kmod
...
var patchFiles = []string{
	...
	"0001-i2c-Fix-shutdown-of-NTC-CHIP-Source-Parts-Popcorn.patch",
...
func find(filename string) (string, error) {
	...
	path := filepath.Join(gopath, "src", "github.com", "jshield", "kernel-ntc-chip", filename)
